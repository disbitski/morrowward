...
Dave chose not to wait for Friday's Apple work before replacing the About-page future-roadmap block. At that point, the page presented the intended destination state: polished **Morrowward for iPhone** and **Morrowward for Mac** source cards plus an active **Follow Dave online** card. Because the Apple source folders did not exist yet, the first two cards were intentionally informational rather than fake links, disabled controls, private-repository URLs, or App Store claims. Their visible status said that the source link was coming after the build. This preserved the design while leaving activation contingent on real source.
...
The root README now makes the companion source prominent, and the dedicated [Apple README](../apple/README.md) documents architecture, XcodeGen regeneration, Release and Debug origins, Simulator use, unsigned Mac builds, security, local persistence, and optional physical-device signing. After commit `d4e0d50` pushed both platform entry points to the private repository, the About panel's iPhone and Mac cards could link to their exact GitHub folders: [`apple/Apps/iPhone`](https://github.com/disbitski/morrowward/tree/main/apple/Apps/iPhone) and [`apple/Apps/Mac`](https://github.com/disbitski/morrowward/tree/main/apple/Apps/Mac). Authorized collaborators can resolve those links now; the same URLs become public with the repository on July 20. Dave's follow-up discernment pass also cut the copy and made the destinations visual: a paired iPhone/iPad glyph for iOS and a clear MacBook silhouette for macOS after the first window-style icon proved ambiguous. They remain source links, not App Store availability claims.
